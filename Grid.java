...
/***
 * Project demonstration link - https://www.youtube.com/watch?v=CS5sN_Az9sI&t=636s
 */
...
public class Grid {
	...
	private void createBombGrid() {          //make it private
		for(int i = 0; i<bombGrid.length; i++)
		{
			for(int j=0; j<bombGrid[i].length; j++)
			{
  ...
  private void createCountGrid() //make it private
  {
		int count =0;
	...
	for(int rows =0; rows< numRows; rows++)
	{
	  for(int cols = 0 ; cols<numColumns; cols++)
	   {
	    // int count = 0;
		if(bombGrid[rows][cols] == true)       //self
		{  
			count++;
			//countGrid[rows][cols] = count;
		}
		if( (rows >0) && bombGrid[rows-1][cols] == true)         //up
		{
			
			count++;
			//countGrid[rows][cols] = count;
		}
	    if(((rows+1 ) <numRows )&&bombGrid[rows+1][cols] == true)                 //down
	    {
	    	
	    	count++;
			//countGrid[rows][cols] = count;
	    }
		if(((cols+1) < numColumns) && bombGrid[rows][cols+1] == true)   // right
		{
		...
		if((rows > 0 && cols > 0) && bombGrid[rows-1][cols-1] == true) // main dia left corner
		{
		...
		if(((rows + 1) < numRows ) && ((cols + 1) < numColumns ) && bombGrid[rows+1][cols+1] == true) //main dia right corner
		{
			count++;
			//countGrid[rows][cols] = count;
		}
		
		if( ((rows + 1) < numRows ) && (cols > 0 ) && bombGrid[rows+1][cols-1] == true) // 2nd dia left corner
		{
			count++;
			//countGrid[rows][cols] = count;
		}
		if((rows > 0  ) && ((cols + 1) < numColumns ) && bombGrid[rows-1][cols+1] == true) //2nd dia right corner
		{
			count++;
			//countGrid[rows][cols] = count;
		}
