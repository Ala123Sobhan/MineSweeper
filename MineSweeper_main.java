...
public class MineSweeper_main extends JFrame{
	...
	public MineSweeper_main(int row, int col, int bombs) {
	...
	private class createBoard extends JPanel implements ActionListener{
		...
		int cell_checked = 0;              //keep track of number of cells revealed
		...
		public void displayBoard() {
			...
			for(int r = 0; r<board.length; r++){
				for(int c = 0; c < board[r].length; c++){
					...
					board[r][c].setBackground(new Color(0,255,127));
		...
		private  void reavel_content () {
			ImageIcon bombImg = new ImageIcon("Image/bomb1.png");
	...
	public void my_recursive(int r, int c)
	{
		...
		{
			//this.reveal_adjacentzeroCell(row_r, col_r);
		    return ;
		}
		if(visited[r][c] == true)
		{
			return ;
			   ...
			   board[r][c].setText(Integer.toString(myGrid.getCountAtLocation(r, c)));   //the zero cells
			   if(board[r][c].isEnabled()) {
				cell_checked++;
				board[r][c].setEnabled(false);
				}
			 
			      my_recursive(r-1, c); //up
			      my_recursive(r+1, c); //down
			      my_recursive(r,c+1);  //right
			      my_recursive(r, c-1); //left
				  my_recursive(r-1, c-1); //main dia top cor
				  my_recursive(r+1,c+1);  //main dia bottom cor
				  my_recursive(r+1,c-1);  //sec dia bottom cor
				  my_recursive(r-1, c+1); //sec dia top cor
		...
		public void actionPerformed(ActionEvent e) {
			...
			ImageIcon sadImg = new ImageIcon("Image/bomb.png");
			ImageIcon scalesadImg = new ImageIcon(sadImg.getImage().getScaledInstance(80,80, Image.SCALE_FAST));
			    ...
			    else if(!myGrid.isBombAtLocation(row, col))            // DOES NT HAVE BOMB
	            {
			    	...
			    	if(board[row][col].isEnabled()) {         // first time check -- it will be true
				    	cell_checked++;
			    	...
			    	this.my_recursive(row, col);     //send the 0 containing cell
			    	}
